...
If you're using Microsoft Intune to manage your devices, there's native integration for [creating and deploying Microsoft 365 Apps](/intune/intune-service/apps/apps-add-office365), including the option to [provide a custom configuration XML](/intune/intune-service/apps/apps-add-office365#step-2---option-2-configure-app-suite-using-xml-data). When creating the app consider setting the following options:
...
For more information on deploying a CMG in Configuration Manager, see the following article: [Plan for the cloud management gateway in Configuration Manager](/intune/configmgr/core/clients/manage/cmg/plan-cloud-management-gateway).
...
For more information on enabling the Client Peer Cache in Configuration Manager, see the following article: [Peer cache for Configuration Manager clients](/intune/configmgr/core/plan-design/hierarchy/client-peer-cache).

#### Enabling BranchCache
To get started with BranchCache refer to the documented [requirements](/intune/configmgr/core/plan-design/configs/support-for-windows-features-and-networks#requirements-to-support-branchcache-with-configuration-manager), then proceed with the following steps to enable the feature:
...
With BranchCache enabled on your distribution points, you'll need to configure and assign the necessary [client settings](/intune/configmgr/core/clients/deploy/about-client-settings#configure-branchcache).
